...
interface SidebarNavProps extends ISidebarProps {
  collapsed: boolean;
}

export function SidebarNav({ closeMobile, collapsed }: SidebarNavProps) {
    ...
    () =>
      sidebarNav
        ...
        .map((item) => ({
          ...
          badge:
            item.badgeCount > 0 ? (
              <GBadge size={SizeEnum.sm} className={collapsed ? "absolute top-1 right-1 min-w-4 justify-center" : "ms-auto min-w-5 justify-center"}>
                {item.badgeCount}
              </GBadge>
            ) : undefined,
        })),
    [t, friendRequestCount, unreadMessageCount, gameInvites.length, unreadNotificationCount, activeId, router, closeMobile, collapsed],
  );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 px-3 py-2" aria-label={t.mainNavigation}>
        <GNav items={navItems} orientation={NavOrientationEnum.Vertical} indicator={IndicatorPositionEnum.Start} collapsed={collapsed} />
      </nav>

      {/* Footer */}
      <footer className="border-t border-border">
        <div className={clsx("flex gap-2 pb-safe", collapsed ? "flex-col items-center gap-3 p-2" : "flex-col gap-2 p-3")}>
          ...
          {collapsed ? (
            <>
              {user && (
                <GButton
                  onClick={() => { router.push("/settings"); if (closeMobile) closeMobile(); }}
