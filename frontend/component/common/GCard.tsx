...
const variants = {
  ...
  elevated: "bg-bg-elevated border border-border transition-all duration-300 hover:border-border-light",
  glass: "bg-bg-card border border-border transition-all duration-300 hover:border-border-light",
  interactive:
...
function GCard({ variant = "default", padding = "md", rounded: roundedProp = "lg", className, children, ...props }: GCardProps) {
  return (
    <div className={clsx(variants[variant], paddings[padding], rounded[roundedProp], className)} {...props}>
      {children}
    </div>
  );
}
