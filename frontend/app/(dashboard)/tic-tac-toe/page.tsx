...
export default function TicTacToePage() {
  ...
  const opponentName = gameState?.player1Id === user?.id 
      ...
      {!roomId && (
        <div className="w-full max-w-md bg-bg-card/70 backdrop-blur-xl border border-border/80 rounded-3xl p-8 shadow-2xl text-center animate-fade-in relative overflow-hidden">
          ...
          <h1 className="text-3xl font-black tracking-tight mb-2 text-white">
            Tic Tac <span className="bg-gradient-to-r from-neon-blue to-neon-purple bg-clip-text text-transparent">Toe</span>
          </h1>
          ...
          {isSearching ? (
            <div className="space-y-6 animate-fade-in">
              ...
              <div>
                <p className="text-sm font-semibold text-white animate-pulse">Searching for opponent...</p>
                <p className="text-xs text-text-muted mt-1">Checking GameArena servers</p>
              </div>
              <button 
                onClick={resetGame}
      ...
      {roomId && (
        <div className="w-full max-w-lg space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-7 items-center bg-bg-card/50 backdrop-blur-xl border border-border/60 rounded-2xl p-4 shadow-lg">
            ...
            <div className="col-span-3 flex flex-col items-center text-center p-2 relative">
              <div className={`relative w-16 h-16 rounded-xl flex items-center justify-center border-2 transition-all duration-300 ${
                gameState?.currentTurnPlayerId === gameState?.player1Id && !gameState?.isFinished
              ...
              <span className="text-sm font-semibold text-white mt-3 truncate max-w-full">
                {gameState?.player1Id === user?.id ? `${myName} (You)` : gameState?.player1Username || "Player 1"}
              </span>
              {gameState?.currentTurnPlayerId === gameState?.player1Id && !gameState?.isFinished && (
            ...
            <div className="col-span-3 flex flex-col items-center text-center p-2 relative">
              <div className={`relative w-16 h-16 rounded-xl flex items-center justify-center border-2 transition-all duration-300 ${
                gameState?.currentTurnPlayerId === gameState?.player2Id && !gameState?.isFinished
              ...
              <span className="text-sm font-semibold text-white mt-3 truncate max-w-full">
                {gameState?.player2Id === user?.id ? `${myName} (You)` : gameState?.player2Username || "Player 2"}
              </span>
              {gameState?.currentTurnPlayerId === gameState?.player2Id && !gameState?.isFinished && (
          ...
          {!gameState?.isFinished && (
            <div className={`w-full py-3 px-4 rounded-xl border text-center font-bold text-sm shadow-md transition-all flex items-center justify-center gap-2 ${
              isMyTurn
                ? "bg-primary-muted border-primary/30 text-white animate-pulse"
                : "bg-surface border-border text-text-secondary"
            }`}>
              <Zap className={`w-4 h-4 ${isMyTurn ? "text-neon-cyan" : "text-text-muted"}`} />
              {isMyTurn ? "Your Turn - Make your move!" : `Waiting for ${opponentName}...`}
            </div>
          )}
          ...
          <div className="relative">
            <div className="grid grid-cols-3 gap-3 bg-bg-card/75 backdrop-blur-xl border border-border/80 rounded-3xl p-5 shadow-2xl relative">
              {board.map((cell, i) => {
                const isCellActive = !cell && isMyTurn && !gameState?.isFinished;
                return (
                  <button
                    key={i}
                  ...
                  >
                    {/* Symbol Render */}
                    {cell === "X" && (
                      <span className="drop-shadow-[0_0_8px_rgba(0,210,255,0.5)] animate-fade-in">X</span>
                    )}
                    {cell === "O" && (
                      <span className="drop-shadow-[0_0_8px_rgba(224,64,251,0.5)] animate-fade-in">O</span>
                    )}
            ...
            {gameState?.isFinished && (
              <div className="absolute inset-0 bg-bg-dark/95 backdrop-blur-sm rounded-3xl border border-border flex flex-col items-center justify-center p-6 text-center animate-fade-in">
                ...
                <h2 className={`text-2xl font-black mt-4 ${getWinnerMessage().color}`}>
